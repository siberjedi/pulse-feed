(() => {
  ...
  function normalizeProviderEndpoint(raw) {
    ...
    if (!value.includes('/api/users')) return value;
    try {
      const u = new URL(value);
      const target = u.searchParams.get('target');
      if (target) {
        return `${u.origin}/relay?target=${encodeURIComponent(target)}`;
      }
  ...
  async function callProvider(provider, prompt, opts = {}) {
    ...
    const normalizedEndpoint = normalizeProviderEndpoint(c.endpoint);
    const endpoint = resolveEndpoint(normalizedEndpoint, c.proxyUrl);
  ...
  function renderAdmin() {
    ...
    wrap.addEventListener('input', (e) => {
      ...
      state.config[id] = {
        ...
        endpoint: normalizeProviderEndpoint(byId(`endpoint-${id}`).value.trim() || item.defaultEndpoint),
    ...
    byId('saveConfigBtn').addEventListener('click', () => {
      for (const p of PROVIDERS) {
        state.config[p.id] = {
          ...
          endpoint: normalizeProviderEndpoint(byId(`endpoint-${p.id}`).value.trim() || p.defaultEndpoint),
